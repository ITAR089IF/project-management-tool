class UserInfo
  ...
  def report
    ...
    array_of_dates.each do |date|
      created_tasks = @user.created_tasks.created_on_date(date).count
      assigned_tasks = @user.assigned_tasks.assigned_on_date(date).count
      completed_tasks = @user.completed_tasks.completed_on_date(date).count
