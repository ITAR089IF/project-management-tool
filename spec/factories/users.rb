    ...
    after(:create) do |user|
      user.workspaces.each do |workspace|
        workspace.projects.each do |project|
          ...
          FactoryBot.create_list(:task, 10, project: project, creator: user)
          FactoryBot.create_list(:task, 10, :random_completed_in_range, project: project, assignee: user, creator: user)
          FactoryBot.create_list(:task, 10, :random_completed_in_range, project: project, creator: user)
        end
      end
    end
  end
